...
health_control <- fread("220218_feature_matrix_no_bert_health_comparator.csv") %>% 
  clean_names()
...
## Issue with interrogative/numbers/quantifiers so skipping for now
for (val in liwc_vader[34:97]) {
  print(val)
...
nonzero_model_health_control_to_export <- nonzero_model_health_control %>%
    filter(term != "(Intercept)") %>%
       mutate(estimate = round(estimate, 2),
              ...
              p.value.adjust = round(p.value.adjust, 3))
setwd("~/Documents/dartmouth/research/aim3_facebook_covid19_obesity/data")
write.csv(nonzero_model_health_control_to_export, "220218_nonzero_model_health_control_to_export_facebook.csv")
write.csv(zero_model_health_control_to_export, "220218_zero_model_health_control_to_export_facebook.csv")
...
nonzero_model_health_control_to_export %>%
  ...
  ungroup() %>%
  #filter(!(val %in% c("filler")) &
  #         !(facet_plot %in% c("Summary", "General"))) %>%
  ggplot(aes(x = term_plot,
             y = val,
             fill = estimate_plot)) + 
  geom_tile() +
  facet_wrap(~facet_plot, scales = "free_y", ncol = 4) + 
  theme_classic() + 
...
setwd("~/Documents/dartmouth/research/aim3_facebook_covid19_obesity/figures")
ggsave("220223_standardized_hurdle_health_control.tiff",
       width = 8.5,
       height = 5.29)
```
