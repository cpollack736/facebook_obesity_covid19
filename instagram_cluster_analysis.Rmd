...
obesity_topics %<>%
  filter(topics >= 2 & topics <= 14 & !(topics %in% c(8, 9, 11))) #17,907 posts
```
...
obesity_topics %>%
  ...
  mutate(topic_renamed = factor(topic_renamed,
                                    levels = c("Topic: 2", "Topic: 3", "Topic: 4", "Topic: 5", "Topic: 6", "Topic: 7", "Topic: 10", "Topic: 12", "Topic: 13", "Topic: 14"))) %>%
...
setwd("~/Documents/dartmouth/research/aim3_facebook_covid19_obesity/figures")
ggsave("220304_topics_per_day_facet_instagram.tiff",
       width = 7.25, 
       height = 4.51)
```
...
dfs <- c("topics_per_day_2", "topics_per_day_3", "topics_per_day_4", "topics_per_day_5", "topics_per_day_6", "topics_per_day_7") #Not running ARIMA on 12, 13, 14 -- too few posts
```
...
for (df in dfs) {
  for (d in list_of_its_dates) {
    if (d == 18281 & df == "topics_per_day_6" |
        d == 18332 & df == "topics_per_day_6") {
      next
    }
    print(as.Date(d))
    print(df)
